...
class ChangePasswordView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(data= {'detail': 'change password successfuly','token':token.key}, status=status.HTTP_200_OK)
...
#     def get(self, request, *args, **kwargs):
#         user = get_object_or_404(CustomeUser, email = "fatemeh@admin.com")
#         token = self.get_tokens_for_user(user)
#         message = EmailMessage('email/email.html', {'user': 'admin', "token":token}, 'fatemeh@admin.com', to=["fatemeh@admin.com"])
#         email = SendEmailWithThreading(message)
#         email.start()
#         return Response({"detail" : "email sent for your verification...!"})
...
class IsVerified(GenericAPIView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            user.save()
            return Response({
                "detail" : "your account veridied successfully"
                })
        except:
            return Response({
